...
After deployment, only the admin account exists:

| Role | ID | Password | Notes |
|------|----|----------|-------|
| Admin | `admin` | Set via `ADMIN_PASSWORD` env var | ⚠️ Change immediately after first login |

**Members**: No demo members exist. Create members through Admin Dashboard → Members → Register New Member.

**Security**: Always use strong passwords in production!
